...
const create_count_stream = (resp, limit)=>new stream.Transform({
    transform(data, encoding, cb){
        if (resp.body_size<limit)
            resp.body.push(data.slice(0, limit-resp.body_size));
...
E.prototype.usage = function(response){
    ...
    const response_body = is_ssl ?
        'unknown' : decode_body(response.body, encoding, this.opt.ssl_limit);
    const data = {
        uuid: response.uuid,
        port: this.port,
        ...
        proxy_peer: headers['x-hola-ip'] || headers['x-luminati-ip'],
...
E.prototype.resolve_proxy = etask._fn(function*resolve_proxy(_this){
    ...
    while (proxies.length && !Object.keys(hosts).length &&
        Date.now()-timestamp<30*SEC)
    {
        ...
        try {
            ...
            _this.logger.warn('Failed to resolve %s (%s): %s', proxy, domain,
                e.message);
        }
    }
    _this.hosts = _.shuffle(Object.keys(hosts));
    if (!_this.hosts.length)
        _this.logger.error('Failed to resolve any proxies');
...
function*request_handler(_this, req, res, proxy, head, headers){
    ...
    const ensure_end_task = ()=>set_timeout(()=>{
...
function*_request_new_socket(_this, task, req, res, head, host){
    ...
    const ip = conn.res.headers['x-hola-ip'] ||
        conn.res.headers['x-luminati-ip'];
    ...
    _headers)
{
    return proxy_res=>{
        ...
        ctx.responded = true;
        const count$ = create_count_stream(ctx.response, this.opt.ssl_limit);
        try {
            ...
            const ip = proxy_res.headers['x-hola-ip'] ||
                proxy_res.headers['x-luminati-ip'];
            ...
            if (!res.resp_written)
            {
                ...
                else if (this.rules.post_need_body(req))
                {
                    ...
                    proxy_res.once('end', ()=>{
                        ...
                        const ssl_limit = this.opt.ssl_limit;
                        for (let i=0; i<temp_data.length; i++)
                        {
                            if (ctx.response.body_size>=ssl_limit || has_body)
                                break;
                            const l = ssl_limit-ctx.response.body_size;
...
E.prototype.handle_proxy_connect = function(req, res, proxy, task, head){
    return (proxy_res, proxy_socket, proxy_head)=>{
        ...
        try {
            ...
            // Close instead of end to prevent 'This socket has been ended by the other party'
            res.once('close', ()=>{
